...
class MLPActor(nn.Module):
    def __init__(self, ob_space, ac_space):
        ...
        # self.transformerblock2 = TransformerBlock(d=ob_space[1])

        self.ac_head = nn.Linear(ob_space[1], 1)
        self.ac_head.weight.data.uniform_(-3e-3, 3e-3)
        self.ac_head.bias.data.uniform_(-3e-3, 3e-3)
        self.ac_space = ac_space

        # multihead attention
        # self.multiheadattention = nn.MultiheadAttention(embed_dim=ob_space[1], num_heads=2, batch_first=True)


    def forward(self, obs):
        # obs = self.multiheadattention(obs, obs, obs, need_weights=False)[0]
